...
export async function POST(request:Request){
  ...
  const db=await getDb();const [existing]=await db.select().from(userActivity).where(and(eq(userActivity.userEmail,user.email),eq(userActivity.activityDay,day))).limit(1);

...
export async function GET(request: Request) {
  ...
  return Response.json({ profile: profile ? { account:profile.email,displayName: profile.displayName, bio: profile.bio, avatar: profile.avatar, avatarUrl:profile.avatarKey?`/api/media?key=${encodeURIComponent(profile.avatarKey)}`:null,followerCount:followerRows.length,isFollowing:!!myFollow,isSelf:user?.email===profile.email,isOnline,todayFishCount:activity?.fishCount||0,todayFishSeconds:activity?.fishSeconds||0 } : { account:null,displayName: resolvedName, bio: "这个人正在认真摸鱼和创造。", avatar: "🐟",avatarUrl:null,followerCount:0,isFollowing:false,isSelf:false,isOnline:false,todayFishCount:0,todayFishSeconds:0 }, works: rows.filter(w => w.status === "published").map(w => ({ id:w.id,title:w.title,description:w.description,type:w.type,authorName:w.authorName,status:w.status,externalUrl:w.externalUrl,coverUrl:w.coverKey?`/api/media?key=${encodeURIComponent(w.coverKey)}`:null })) });
}

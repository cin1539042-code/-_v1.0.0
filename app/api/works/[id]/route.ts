...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  return Response.json({ work: { id: work.id, title: work.title, description: work.description, type: work.type, authorName: work.authorName, content: work.content, fileName: work.fileName, status:work.status, externalUrl:work.externalUrl, windowSize:work.windowSize,windowWidth:work.windowWidth,windowHeight:work.windowHeight,coverUrl:work.coverKey?`/api/media?key=${encodeURIComponent(work.coverKey)}`:null, createdAt: work.createdAt, appUrl, appHtml: appHtml || work.content } });
}
...
export async function PUT(request:Request, context:{params:Promise<{id:string}>}) {
  ...
  const [work]=await db.update(works).set({title,type,description:String(form.get("description")||"").trim().slice(0,240),content,fileKey,fileName,coverKey,externalUrl,windowSize,windowWidth,windowHeight,status:form.get("status")==="published"?"published":"draft",updatedAt:new Date().toISOString()}).where(eq(works.id,old.id)).returning();

...
export async function GET(_request:Request,context:{params:Promise<{token:string;path:string[]}>}){
  ...
  return new Response(body,{headers:{"content-type":object.httpMetadata?.contentType||contentTypeFor(safe),"cache-control":isHtml?"no-store":"private, max-age=3600","x-content-type-options":"nosniff"}})
}

...
export async function GET(request:Request,context:{params:Promise<{appId:string;version:string;path:string[]}>}){
  ...
  const etag=`"${object.etag}"`;if(!isHtml&&request.headers.get("if-none-match")===etag)return new Response(null,{status:304,headers:{etag,"cache-control":"public, max-age=31536000, immutable"}});
  return new Response(body,{headers:{"content-type":object.httpMetadata?.contentType||contentTypeFor(safe),"cache-control":isHtml?"no-store":"public, max-age=31536000, immutable",etag,"x-content-type-options":"nosniff"}})
}

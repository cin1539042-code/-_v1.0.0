...
type Work = { id:number; type:string; title:string; description:string; authorName:string; authorAvatar?:string|null; authorEmoji?:string; status?:string; content?:string; appHtml?:string; externalUrl?:string|null; coverUrl?:string|null; windowSize?:string;windowWidth?:number;windowHeight?:number;playCount?:number; favoriteCount?:number; isFavorited?:boolean; updatedAt?:string };
...
const formatDate=(value?:string)=>value?new Date(value).toLocaleDateString("zh-CN",{year:"numeric",month:"2-digit",day:"2-digit"}):"暂无";
const beijingDay=()=>new Intl.DateTimeFormat("en-CA",{timeZone:"Asia/Shanghai",year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date());
...
export default function CommunityApp({user}:{user:User}) {
  ...
  const emptyForm={id:0,title:"",type:"",description:"",content:"",externalUrl:"",windowSize:"desktop",windowWidth:1200,windowHeight:800};
  ...
  const loadFavorites=async()=>{if(!user)return;const r=await fetch("/api/works?favorite=1");const d=await r.json();if(r.ok)setFavoriteWorks(d.works)};
  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(()=>{void loadWorks();if(user){void loadProfile();void loadMine()}},[]);
  useEffect(()=>{let day=beijingDay();setMoyuSeconds(Number(localStorage.getItem(`moyu-seconds:${day}`)||0));setTodayFish(Number(localStorage.getItem(`moyu-clicks:${day}`)||0));const timer=window.setInterval(()=>{const current=beijingDay();if(current!==day){day=current;setMoyuSeconds(0);setTodayFish(0)}else setMoyuSeconds(x=>{const next=x+1;localStorage.setItem(`moyu-seconds:${day}`,String(next));return next});setClockNow(new Date())},1000);return()=>window.clearInterval(timer)},[]);
  useEffect(()=>{const check=async()=>{try{const d=await fetch("/api/version",{cache:"no-store"}).then(r=>r.json());const seen=sessionStorage.getItem("moyu-site-version");if(seen&&seen!==d.version){setUpdateAvailable(true);if(tab!=="创作中心"||(!form.title&&!form.content))window.setTimeout(()=>location.reload(),1500)}sessionStorage.setItem("moyu-site-version",d.version)}catch{}};void check();const timer=window.setInterval(check,20000);return()=>window.clearInterval(timer)},[tab,form.title,form.content]);
  ...
  const copyStorageExample=async()=>{try{await navigator.clipboard.writeText(`// 保存进度\nawait MoyuStorage.set("progress", { level: 2, score: 100 });\n\n// 恢复进度\nconst progress = await MoyuStorage.get("progress");`);setCopyNotice("✓ 复制成功，可以粘贴到作品代码中");window.setTimeout(()=>setCopyNotice(""),3000)}catch{setCopyNotice("复制失败，请手动选择代码")}};
  const catchFish=()=>{const day=beijingDay();setTodayFish(x=>{const next=x+1;localStorage.setItem(`moyu-clicks:${day}`,String(next));return next})};
  const allWorks=works.filter(w=>(category==="全部"||w.type===category)&&`${w.title}${w.description}${w.type}${w.authorName}`.toLowerCase().includes(query.toLowerCase()));

  const openWork=async(w:Work)=>{void fetch(`/api/works/${w.id}/play`,{method:"POST"}).then(()=>loadWorks());if(w.externalUrl){window.open(w.externalUrl,"_blank","noopener,noreferrer");return}const r=await fetch(`/api/works/${w.id}`);const d=await r.json();if(r.ok){if(d.work.externalUrl)window.open(d.work.externalUrl,"_blank","noopener,noreferrer");else{setViewerMode(d.work.windowSize||"desktop");setMinimized(false);setViewer(d.work)}}else setMessage(d.error||"加载失败")};
  const openCreator=async(name:string)=>{const r=await fetch(`/api/profile?name=${encodeURIComponent(name)}`);const d=await r.json();if(r.ok)setCreator(d)};
  const toggleFavorite=async(w:Work)=>{if(!user){setShowLogin(true);return}const r=await fetch(`/api/works/${w.id}/favorite`,{method:"POST"});const d=await r.json();if(!r.ok){setMessage(d.error||"收藏失败");return}setWorks(list=>list.map(x=>x.id===w.id?{...x,isFavorited:d.favorited,favoriteCount:d.favoriteCount}:x));await loadFavorites()};
  const changeTab=(next:string)=>{setTab(next);if(next==="我的作品")void loadMine();if(next==="我的收藏")void loadFavorites();if(next==="个人主页")void loadProfile()};
  ...
  const editWork=async(w:Work)=>{const r=await fetch(`/api/works/${w.id}`);const d=await r.json();if(!r.ok){setMessage(d.error||"加载失败");return}setForm({id:w.id,title:d.work.title,type:d.work.type,description:d.work.description||"",content:d.work.appHtml||d.work.content||"",externalUrl:d.work.externalUrl||"",windowSize:d.work.windowSize||"desktop",windowWidth:d.work.windowWidth||1200,windowHeight:d.work.windowHeight||800});changeTab("创作中心")};
  ...
  return <main>
    <header className="topbar"><button className="brand" onClick={()=>changeTab("发现功能")}><span className="fish">🐟</span><span>摸鱼开发广场</span></button><nav>{["发现功能","创作中心","我的作品","我的收藏"].map(x=><button key={x} className={tab===x?"active":""} onClick={()=>changeTab(x)}>{x}</button>)}</nav><div className="fish-counter"><span>今日摸鱼</span><b>{todayFish}</b><small>次</small></div><div className="moyu-timer"><i>●</i><span>您已摸鱼</span><b>{moyuTime}</b></div>{user?<button className="profile" onClick={()=>changeTab("个人主页")} title="打开个人主页">{profile.avatarUrl?<img src={profile.avatarUrl} alt="头像"/>:profile.avatar}<span className="online"/></button>:<button className="login" onClick={()=>setShowLogin(true)}>登录 / 注册</button>}</header>
    <div className="moyu-background" aria-hidden="true"><div className="wall-clock"><i className="hour-hand" style={{transform:`rotate(${clockNow.getHours()%12*30+clockNow.getMinutes()*.5}deg)`}}/><i className="minute-hand" style={{transform:`rotate(${clockNow.getMinutes()*6}deg)`}}/><i className="second-hand" style={{transform:`rotate(${clockNow.getSeconds()*6}deg)`}}/><b>12</b><b>3</b><b>6</b><b>9</b></div>{["🐟","🐠","🐡","🐟","🐠","🐟","🐡"].map((fish,i)=><button key={i} className={`swimming-fish fish-${i+1}`} onClick={catchFish} aria-label="摸鱼一次">{fish}</button>)}</div>
    {tab==="创作中心"&&<aside className="storage-notice"><div><span>NEW</span><b>💾 平台本地存档已经开放</b><p>小游戏进度、阅读位置和工具设置现在都能保留。发布流程不变，按需调用 <code>MoyuStorage</code> 即可。</p></div><button onClick={()=>setShowStorageDocs(true)}>查看接入文档 →</button></aside>}
    {tab==="创作中心"&&<aside className="size-recommender"><div><b>作者推荐窗口</b><p>选择常用尺寸，或输入自由宽高。右侧会实时展示窗口比例。</p><div className="size-presets">{[["desktop","桌面",1200,800],["tablet","平板",820,720],["mobile","手机",390,760],["mini","小窗",520,420]].map(([key,label,w,h])=><button key={String(key)} className={form.windowSize===key?"active":""} onClick={()=>setForm(f=>({...f,windowSize:String(key),windowWidth:Number(w),windowHeight:Number(h)}))}>{String(label)}<small>{String(w)}×{String(h)}</small></button>)}</div><div className="custom-dimensions"><button className={form.windowSize==="custom"?"active":""} onClick={()=>setForm(f=>({...f,windowSize:"custom"}))}>自由尺寸</button><label>宽 <input type="number" min="320" max="1600" value={form.windowWidth} onChange={e=>setForm(f=>({...f,windowSize:"custom",windowWidth:Number(e.target.value)}))}/></label><span>×</span><label>高 <input type="number" min="300" max="1000" value={form.windowHeight} onChange={e=>setForm(f=>({...f,windowSize:"custom",windowHeight:Number(e.target.value)}))}/></label></div></div><div className="size-preview"><div style={{aspectRatio:`${form.windowWidth}/${form.windowHeight}`}}><span/><b>{form.windowWidth} × {form.windowHeight}</b><small>打开时的推荐比例</small></div></div></aside>}
    {copyNotice&&<div className="copy-success">{copyNotice}</div>}
    {updateAvailable&&<button className="update-toast" onClick={()=>location.reload()}>发现新版本，正在自动刷新… <b>立即刷新</b></button>}

    {tab==="发现功能"&&<><section className="hero"><div className="hero-copy"><span className="eyebrow">CREATE WITHOUT LIMITS</span><h1>上班摸个鱼，<br/>顺手<span>造点好玩的</span></h1><p>不限制题材和类型。游戏、音乐、小说、效率工具，或者从未有人做过的新点子，都可以发布给大家使用。</p><div className="search"><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="搜索作品、标签或创作者..."/></div><div className="hero-actions"><button className="primary" onClick={()=>changeTab("创作中心")}>✎ 自由创作</button><button className="secondary" onClick={()=>document.getElementById("market")?.scrollIntoView({behavior:"smooth"})}>◉ 看看大家的作品</button></div></div><div className="hero-art"><div className="window"><span/><span/><span/></div><div className="sun">☀</div><div className="plant">🪴</div><div className="desk"/><div className="laptop">&lt;/&gt;</div><div className="mascot">🐠</div><div className="mug">☕</div><div className="note n1">题材不限<br/>自由发挥</div><div className="note n2">灵感 +1<br/>快乐 × N</div></div></section><section className="market" id="market"><div className="section-head"><div><span className="fire">🔥</span><h2>大家正在玩</h2><em>所有题材</em></div><button onClick={loadWorks}>刷新作品 ↻</button></div><div className="cards">{allWorks.map((w,i)=><article className={`card ${["peach","blue","cream","mint","lavender","yellow"][i%6]}`} key={w.id}><div className={`cover ${w.coverUrl?"has-cover":""}`} onClick={()=>openWork(w)}>{w.coverUrl&&<img className="work-cover-image" src={w.coverUrl} alt={`${w.title}封面`}/>}<span className="type">{w.type}</span>{!w.coverUrl&&<div className="cover-icon">{["📖","🕹️","📰","🎵","✨","🧩"][i%6]}</div>}<button className="play">打开使用</button></div><div className="card-body"><div><h3>{w.title}</h3><p>{w.description}</p><div className="work-stats"><span>▶ {w.playCount||0} 次游玩</span><span>更新于 {formatDate(w.updatedAt)}</span></div></div><button className={`favorite-button ${w.isFavorited?"on":""}`} onClick={()=>toggleFavorite(w)} title="收藏作品">{w.isFavorited?"♥":"♡"}<small>{w.favoriteCount||0}</small></button></div><footer><button className="author-link" onClick={()=>openCreator(w.authorName)}>{w.authorAvatar?<img className="avatar-image" src={w.authorAvatar} alt="作者头像"/>:<span className="avatar">{w.authorEmoji||"🐟"}</span>}{w.authorName}</button><span className="use">可直接运行</span></footer></article>)}</div></section></>}

    {tab==="创作中心"&&<section className="workspace studio"><div className="studio-intro"><span className="eyebrow">FREE CREATOR</span><h1>{form.id?"编辑作品":"自由创作台"}</h1><p>题材不限，发布时只需选择一个便于检索的分类。</p></div><div className="editor"><div className="editor-head"><div><b>{form.id?"更新现有作品":"创建新作品"}</b><small>{user?`创作者：${profile.displayName||user.displayName}`:"登录后才能保存和发布"}</small></div></div><label>作品名称 <em>必填</em><input value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))}/></label><label>作品分类 <em>必选</em><select value={form.type} onChange={e=>setForm(f=>({...f,type:e.target.value}))}><option value="">请选择分类</option>{categories.map(x=><option key={x}>{x}</option>)}</select></label><label>推荐打开尺寸 <small>用户打开时仍可自由切换</small><select value={form.windowSize} onChange={e=>setForm(f=>({...f,windowSize:e.target.value}))}><option value="desktop">桌面窗口 1200×800</option><option value="tablet">平板窗口 820×720</option><option value="mobile">手机窗口 390×760</option><option value="mini">小窗 520×420</option></select></label><label>作品介绍<textarea value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))}/></label><div className="upload-zone"><b>作品封面图</b><p>支持 JPG、PNG、WebP 等图片，最大 3MB。</p><input className="file" ref={coverRef} type="file" accept="image/*"/></div><label>已有网页链接 <small>选填；填写后打开作品将直接跳转</small><input type="url" value={form.externalUrl} onChange={e=>setForm(f=>({...f,externalUrl:e.target.value}))} placeholder="https://example.com"/></label><div className="optional-templates"><b>可选起步模板</b><div>{starterTemplates.map(t=><button key={t.name} onClick={()=>setForm(f=>({...f,content:t.code}))}>{t.icon} {t.name}</button>)}</div></div><div className="upload-zone"><b>上传 HTML 作品</b><input className="file" ref={fileRef} type="file" accept=".html,text/html"/></div><label>在线编辑 HTML / CSS / JavaScript<textarea className="code-input" spellCheck={false} value={form.content} onChange={e=>setForm(f=>({...f,content:e.target.value}))}/></label>{message&&<div className="toast">{message}</div>}<div className="publish-bar">{form.id&&<button className="secondary" onClick={()=>setForm(emptyForm)}>取消编辑</button>}<button className="secondary" disabled={saving} onClick={()=>save("draft")}>保存草稿</button><button className="preview-btn" onClick={()=>{if(form.externalUrl)window.open(form.externalUrl,"_blank");else{setViewerMode(form.windowSize);setMinimized(false);setViewer({...form,authorName:user?.displayName||"我",appHtml:form.content})}}}>▶ 预览</button><button className="primary" disabled={saving} onClick={()=>save("published")}>{saving?"正在保存…":form.id?"更新并发布":"发布作品 →"}</button></div></div></section>}
    ...
    {showStorageDocs&&<div className="overlay" onMouseDown={e=>e.target===e.currentTarget&&setShowStorageDocs(false)}><section className="modal storage-docs"><button className="close" onClick={()=>setShowStorageDocs(false)}>×</button><span className="eyebrow">DEVELOPER GUIDE</span><h2>为作品接入本地存档</h2><p>平台会在作品运行时自动提供 <code>window.MoyuStorage</code>。无需申请权限、无需连接数据库，也不会改变上传与发布步骤。</p><div className="docs-grid"><article><b>保存与读取</b><pre>{`await MoyuStorage.set("progress", {\n  level: 2,\n  score: 100\n});\n\nconst data = await MoyuStorage.get("progress");`}</pre></article><article><b>其他操作</b><pre>{`await MoyuStorage.remove("progress");\nawait MoyuStorage.clear();\nconst keys = await MoyuStorage.keys();`}</pre></article></div><ul><li>每个作品拥有独立空间，不能读取其他作品的数据。</li><li>每个作品最多 1MB，单条最大 100KB，最多 100 个键。</li><li>支持 JSON 对象、数组、字符串、数字、布尔值和 null。</li><li>第一版保存在当前浏览器；清除浏览器数据或更换设备后不会同步。</li><li>不需要存档的作品无需修改任何代码。</li></ul><div className="docs-actions"><button className="secondary" onClick={copyStorageExample}>复制接入示例</button><button className="primary" onClick={()=>setShowStorageDocs(false)}>我知道了</button></div></section></div>}
    ...
    {viewer&&<><div className={`overlay app-overlay ${minimized?"window-hidden":""}`} onMouseDown={e=>e.target===e.currentTarget&&setViewer(null)}><section className={`modal app-modal mode-${viewerMode}`} style={viewerMode==="custom"?{width:`min(${viewer.windowWidth||1200}px,96vw)`,height:`min(${viewer.windowHeight||800}px,94vh)`}:undefined}><div className="app-toolbar"><div className="traffic-lights"><button onClick={()=>setViewer(null)} title="关闭"/><button onClick={()=>setMinimized(true)} title="最小化"/><button onClick={()=>setViewerMode("full")} title="全屏"/></div><div className="app-identity"><span>✦</span><div><b>{viewer.title}</b><small>{viewer.authorName} · 本地存档已开启</small></div></div><div className="window-sizes"><button className={viewerMode==="mobile"?"active":""} onClick={()=>setViewerMode("mobile")}>手机</button><button className={viewerMode==="tablet"?"active":""} onClick={()=>setViewerMode("tablet")}>平板</button><button className={viewerMode==="mini"?"active":""} onClick={()=>setViewerMode("mini")}>小窗</button><button className={viewerMode==="desktop"?"active":""} onClick={()=>setViewerMode("desktop")}>桌面</button><button className={viewerMode==="custom"?"active":""} onClick={()=>setViewerMode("custom")}>作者推荐</button><button onClick={()=>setViewerMode("full")}>全屏</button></div><div className="app-toolbar-actions"><button onClick={clearViewerStorage}>清除存档</button><button onClick={()=>setMinimized(true)}>—</button><button onClick={()=>setViewer(null)}>×</button></div></div><iframe ref={appFrameRef} className="app-frame" title={viewer.title} sandbox="allow-scripts" srcDoc={withStorageBridge(viewer.appHtml||viewer.content||"<h1>作品暂无内容</h1>")}/><div className="resize-hint">拖动右下角自由调整</div></section></div>{minimized&&<button className="minimized-app" onClick={()=>setMinimized(false)}><span>✦</span><div><b>{viewer.title}</b><small>点击恢复应用</small></div></button>}</>}
  </main>
}

...
type Work = { id:number; type:string; title:string; description:string; authorName:string; authorAvatar?:string|null; authorEmoji?:string; status?:string; content?:string; appHtml?:string; externalUrl?:string|null; coverUrl?:string|null; windowSize?:string;playCount?:number; favoriteCount?:number; isFavorited?:boolean; updatedAt?:string };
...
export default function CommunityApp({user}:{user:User}) {
  ...
  const emptyForm={id:0,title:"",type:"",description:"",content:"",externalUrl:"",windowSize:"desktop"};
  ...
  const copyStorageExample=async()=>{await navigator.clipboard.writeText(`// 保存进度\nawait MoyuStorage.set("progress", { level: 2, score: 100 });\n\n// 恢复进度\nconst progress = await MoyuStorage.get("progress");`);setMessage("存档示例已复制")};
  ...
  return <main>
    ...
    {viewer&&<><div className={`overlay app-overlay ${minimized?"window-hidden":""}`} onMouseDown={e=>e.target===e.currentTarget&&setViewer(null)}><section className={`modal app-modal mode-${viewerMode}`}><div className="app-toolbar"><div><b>{viewer.title}</b><small>{viewer.type||"自由创作"} · {viewer.authorName} · 本地存档可用</small></div><div className="window-sizes"><button className={viewerMode==="mobile"?"active":""} onClick={()=>setViewerMode("mobile")} title="手机尺寸">手机</button><button className={viewerMode==="tablet"?"active":""} onClick={()=>setViewerMode("tablet")} title="平板尺寸">平板</button><button className={viewerMode==="mini"?"active":""} onClick={()=>setViewerMode("mini")} title="小窗">小窗</button><button className={viewerMode==="desktop"?"active":""} onClick={()=>setViewerMode("desktop")} title="桌面尺寸">桌面</button><button onClick={()=>setViewerMode("full")} title="全屏">全屏</button></div><div className="app-toolbar-actions"><button onClick={clearViewerStorage}>清除存档</button><button onClick={()=>setMinimized(true)}>最小化</button><button onClick={()=>setViewer(null)}>退出 ×</button></div></div><iframe ref={appFrameRef} className="app-frame" title={viewer.title} sandbox="allow-scripts" srcDoc={withStorageBridge(viewer.appHtml||viewer.content||"<h1>作品暂无内容</h1>")}/></section></div>{minimized&&<button className="minimized-app" onClick={()=>setMinimized(false)}><span>✦</span><div><b>{viewer.title}</b><small>点击恢复应用</small></div></button>}</>}
  </main>
}

...
export async function requireAdmin(request:Request){const user=await getChatGPTUser();if(!user)return null;const raw=request.headers.get("cookie")?.split(";").map(x=>x.trim()).find(x=>x.startsWith(`${COOKIE}=`))?.slice(COOKIE.length+1);if(!raw)return null;const [encoded,sig]=raw.split(".");try{const payload=atob(encoded),[email,expires]=payload.split("|");const expected=await signature(payload);const a=new TextEncoder().encode(sig||""),b=new TextEncoder().encode(expected);if(email!==user.email||Date.now()>Number(expires)||a.length!==b.length)return null;let mismatch=0;for(let i=0;i<a.length;i++)mismatch|=a[i]^b[i];if(mismatch)return null;return user}catch{return null}}
export function validWriteOrigin(request:Request){const origin=request.headers.get("origin");return !origin||origin===new URL(request.url).origin}

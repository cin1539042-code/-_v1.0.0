...
export async function ensureMessageSchema() {
  ...
  ready = env.DB.batch([
    env.DB.prepare("CREATE TABLE IF NOT EXISTS direct_messages (id integer PRIMARY KEY AUTOINCREMENT NOT NULL,sender_email text NOT NULL,recipient_email text NOT NULL,content text NOT NULL,created_at text DEFAULT CURRENT_TIMESTAMP NOT NULL,read_at text)"),
